...
function showProfile() {
  ...
  (async () => {
    const res = await msgBg({ type: 'CHECK_PREMIUM' });
    if (res?.ok && res.data?.premium) {
      const profileCard = document.querySelector('.profile-card');
      if (profileCard) {
        let pBadge = profileCard.querySelector('.profile-premium-badge');
        if (!pBadge) {
          pBadge = document.createElement('span');
          pBadge.className = 'profile-premium-badge';
          pBadge.style.cssText = 'background:var(--gold);color:var(--bg);font-size:8px;padding:1px 6px;border-radius:4px;font-weight:600;margin-left:auto';
          pBadge.textContent = 'PRO';
          const logoutBtn = $('btn-logout');
          if (logoutBtn) {
            profileCard.insertBefore(pBadge, logoutBtn);
          }
        }
      }
    }
...
async function loadMonitorStats() {
  ...
  // Check premium status for monitor features
  const premiumRes = await msgBg({ type: 'CHECK_PREMIUM' });
  const isPremium = premiumRes?.ok && premiumRes.data?.premium;
  ...
  // Show premium upsell for non-premium users in Monitor tab
  const monitorSection = $('tab-monitor');
  let premiumUpsell = $('monitor-premium-upsell');
  if (!isPremium) {
    if (!premiumUpsell) {
      premiumUpsell = document.createElement('div');
      premiumUpsell.id = 'monitor-premium-upsell';
      premiumUpsell.style.cssText = 'background:var(--gold-dim);border:1px solid var(--gold-border);border-radius:8px;padding:12px;margin-top:8px;cursor:pointer;text-align:center';
      premiumUpsell.innerHTML = `
        <div style="font-size:14px;margin-bottom:4px">📊</div>
        <div style="font-size:11px;color:var(--gold);font-weight:500">Falah Pro Feature</div>
        <div style="font-size:10px;color:var(--b2);margin-top:2px">Upgrade for 90-day history, export reports, and trends chart.</div>
        <div style="font-size:10px;color:var(--gold);margin-top:4px">✨ Tap to upgrade →</div>
      `;
      premiumUpsell.addEventListener('click', () => switchTab('premium'));
      const hmRecentList = $('hm-recent-list');
      if (hmRecentList?.parentNode) {
        hmRecentList.parentNode.appendChild(premiumUpsell);
      }
    }
    // Show premium badge on title
    const monitorTitle = document.querySelector('#tab-monitor .p-section-title');
    if (monitorTitle && !monitorTitle.querySelector('.pf-badge')) {
      const badge = document.createElement('span');
      badge.className = 'pf-badge';
      badge.style.cssText = 'background:var(--gold-dim);color:var(--gold);font-size:8px;padding:1px 5px;border-radius:4px;margin-left:6px;vertical-align:middle';
      badge.textContent = 'Free: 24h';
      monitorTitle.appendChild(badge);
    }
  } else {
    if (premiumUpsell) premiumUpsell.remove();
  }
  ...
  if (historyList) {
    if (!stats.history || stats.history.length === 0) {
      ...
      historyList.innerHTML = stats.history.slice(0, isPremium ? 20 : 10).map(item => {
...
async function checkPremium() {
  const res = await msgBg({ type: 'CHECK_PREMIUM' });
  return res?.ok ? res.data : { premium: false, tier: 'free', expiresAt: null };
}
...
async function setupPremium() {
  ...
  // Subscribe buttons
  $('btn-upgrade-monthly')?.addEventListener('click', () => {
    chrome.tabs.create({ url: 'https://falah-os.com/premium?plan=monthly' });
  });
  $('btn-upgrade-annual')?.addEventListener('click', () => {
    chrome.tabs.create({ url: 'https://falah-os.com/premium?plan=annual' });
  });

  // License key application
  $('btn-apply-license')?.addEventListener('click', applyLicenseKey);
...
async function applyLicenseKey() {
  ...
  const msgEl = $('license-message');
  const key = input?.value.trim();
  if (!key) {
    if (msgEl) { msgEl.textContent = 'Please enter a license key.'; msgEl.style.color = 'var(--ruby)'; }
    return;
  }

  if (msgEl) { msgEl.textContent = 'Validating...'; msgEl.style.color = 'var(--b2)'; }

  const res = await msgBg({ type: 'ACTIVATE_PREMIUM', licenseKey: key });
  if (msgEl) {
    if (res?.ok) {
      msgEl.textContent = '✅ Premium activated! Your subscription is valid for 1 year.';
      msgEl.style.color = 'var(--jade)';
      ...
      msgEl.style.color = 'var(--ruby)';
    }
  }
}

function updatePremiumUI(status) {
  const iconEl = $('premium-icon');
  const labelEl = $('premium-tier-label');
  const textEl = $('premium-status-text');
  const ctaEl = $('premium-cta-area');

  if (!iconEl || !labelEl || !textEl) return;

  if (status.premium) {
    iconEl.textContent = '👑';
    labelEl.textContent = status.tier === 'family' ? 'Falah Family' : 'Falah Pro ✓';
    labelEl.style.color = 'var(--gold-light)';
    textEl.textContent = status.expiresAt
      ? `Active until ${new Date(status.expiresAt).toLocaleDateString()}`
      : 'All premium features unlocked. JazakAllah khair for your support!';
    textEl.style.color = 'var(--jade)';
    if (ctaEl) {
      ctaEl.innerHTML = '<div style="font-size:11px;color:var(--b2);margin-top:8px">Thank you for being a premium supporter. 🕌</div>';
    }
  } else {
    iconEl.textContent = '⭐';
    labelEl.textContent = 'Upgrade to Falah Pro';
    labelEl.style.color = 'var(--gold-light)';
    textEl.textContent = 'Unlock unlimited NurBuddy chat, 90-day Halal Monitor, Adhan audio, and more.';
    textEl.style.color = 'var(--b2)';
    if (ctaEl) {
      ctaEl.innerHTML = '<div style="font-size:11px;color:var(--gold);margin-top:8px;cursor:pointer" id="premium-upsell-link">✨ See premium features above</div>';
      $('premium-upsell-link')?.addEventListener('click', () => switchTab('premium'));
    }
  }
}

// ── NurBuddy (Premium-Gated) ─────────────────────────────────────────────────

const NURBUDDY_DAILY_LIMIT = 20;
...
function setupNurBuddy() {
  ...
  // Check premium status and add limit indicator
  (async () => {
    const res = await msgBg({ type: 'IS_FEATURE_AVAILABLE', feature: 'nurbuddy' });
    if (res?.ok) {
      if (!res.data.premium) {
        // Show daily limit indicator for free users
        const indicator = document.createElement('div');
        indicator.id = 'nb-limit-indicator';
        indicator.style.cssText = 'font-size:10px;color:var(--b3);text-align:center;padding:4px;border-bottom:1px solid var(--bm);cursor:pointer';
        indicator.innerHTML = `💬 Free: ${NURBUDDY_DAILY_LIMIT} msgs/day · <span style="color:var(--gold)">Upgrade for unlimited →</span>`;
        indicator.addEventListener('click', () => switchTab('premium'));
        nbChatArea.parentNode.insertBefore(indicator, nbChatArea);
      }
    }
  })();
  ...
  async function sendMsg() {
    ...
    try {
      // Use premium-gated NURBUDDY_SEND instead of raw API call
      const res = await msgBg({ type: 'NURBUDDY_SEND', query: text, userId: TEST_USER_ID });
      ...
      if (res?.ok && res.data) {
        ...
        // Update daily limit indicator for free users
        if (typeof res.remainingDaily === 'number') {
          const indicator = $('nb-limit-indicator');
          if (indicator) {
            indicator.innerHTML = `💬 Free: ${res.remainingDaily}/${NURBUDDY_DAILY_LIMIT} msgs left today · <span style="color:var(--gold)">Upgrade for unlimited →</span>`;
          }
        }
      } else {
        nbChatArea.innerHTML += `
        ...
        // If premium required, add upgrade link
        if (res?.premiumRequired) {
          nbChatArea.innerHTML += `
            <div class="nb-msg nb-msg-ai">
              <div class="nb-msg-icon">⭐</div>
              <div class="nb-msg-bubble" style="color:var(--gold);cursor:pointer" id="nb-premium-upsell">
                ✨ You've reached today's free limit. <u>Upgrade to Falah Pro</u> for unlimited NurBuddy chats!
              </div>
            </div>
          `;
          $('nb-premium-upsell')?.addEventListener('click', () => switchTab('premium'));
        }
      }
    } catch (e) {
      const loadingEl = $(loadingId);
